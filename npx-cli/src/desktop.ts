...
async function installAndLaunchWindows(
  ...
  const sentinel = readSentinel(dir);
  if (sentinel?.appPath) {
    const appExe = path.join(sentinel.appPath, 'GenCap Control Room.exe');
  ...
  console.error('Installing GenCap Control Room...');
  try {
    ...
    if (fs.existsSync(path.join(defaultDir, 'GenCap Control Room.exe'))) {
      writeSentinel(dir, {
        type: 'nsis-exe',
        appPath: defaultDir,
      });
      return launchWindowsApp(
        path.join(defaultDir, 'GenCap Control Room.exe')
      );
    }
    console.error(
      'Installation complete. Please launch GenCap Control Room from your Start menu.'
    );
    return 0;
  }
  ...
  const appExe = path.join(installDir, 'GenCap Control Room.exe');
  if (fs.existsSync(appExe)) {
    return launchWindowsApp(appExe);
  }
...
function launchWindowsApp(appExe: string): number {
  console.error('Launching GenCap Control Room...');
