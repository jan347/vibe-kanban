...
GenCap Control Room Cloud lets you collaborate with your team in real-time. Your data syncs to the cloud, so everyone sees the same projects, issues, and updates.

<Info>
**Already using GenCap Control Room locally?** Cloud uses the same interface you're familiar with. The difference is your data syncs to the cloud and you can invite team members.
</Info>

## Step 1: Launch GenCap Control Room

If you haven't already, start GenCap Control Room:
...
<Step title="Authorise the application">
  You'll be redirected to GitHub or Google. Sign in if needed, then click **Authorize** or **Allow** to grant GenCap Control Room access.

  <Info>
  GenCap Control Room only requests access to your basic profile (name, email, avatar). It cannot access your private repositories or post on your behalf.
  </Info>
</Step>

<Step title="Return to GenCap Control Room">
...
Both appear in the same GenCap Control Room interface. Local projects show in one section, Cloud projects in another.
