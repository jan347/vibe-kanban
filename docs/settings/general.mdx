...
When running GenCap Control Room on a remote server (e.g., accessed via Cloudflare tunnel, ngrok, or as a systemctl service), you can configure VSCode-based editors to open projects via SSH instead of assuming localhost.
...
- GenCap Control Room runs on a remote server (VPS, cloud instance, etc.)
...
Set a prefix for auto-generated branch names. When you create a new workspace or task, GenCap Control Room automatically creates a git branch for your changes.
...
Specify where GenCap Control Room stores workspace data. Workspaces are created in a `.gencap-workspaces` subdirectory within this path.
...
Configure how GenCap Control Room alerts you about task progress and status changes.

- **Sound Effects** - Play audio notifications when tasks complete, need attention, or encounter errors. Useful when working with multiple tasks or when GenCap Control Room runs in a background tab.
- **Push Notifications** - Receive browser notifications even when GenCap Control Room isn't in focus. Requires browser permission when first enabled.
...
Enable or disable telemetry data collection to help improve GenCap Control Room.
