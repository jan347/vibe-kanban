---
title: "GenCap Control Room MCP Server"
description: "Configure the GenCap Control Room MCP server"
---

GenCap Control Room exposes a local MCP (Model Context Protocol) server, allowing you to manage organisations, projects, issues, workspaces, and repositories from external MCP clients like Claude Desktop, Raycast, or even coding agents running within GenCap Control Room itself.

<Note>
This page covers connecting **external MCP clients** to GenCap Control Room's MCP server. For configuring MCP servers **within** GenCap Control Room for your coding agents, see the [MCP Server Configuration](/integrations/mcp-server-configuration) guide.
</Note>

<Info>
GenCap Control Room's MCP server is **local-only** - it runs on your computer and can only be accessed by applications installed locally. It cannot be accessed via publicly accessible URLs.
</Info>
...
This works if you're adding the GenCap Control Room MCP server to any [supported coding agent](/supported-coding-agents) **within** GenCap Control Room.

1. In GenCap Control Room Settings, navigate to the "MCP Servers" page
2. In the "Popular servers" section, click on the GenCap Control Room card
3. Click the `Save Settings` button

<Frame>
<img src="/images/vk-mcp-server-config.png" alt="MCP Servers configuration page showing how to add GenCap Control Room MCP server" />
</Frame>
...
```json
{
  "mcpServers": {
    "gencap": {
      "command": "npx",
      "args": ["-y", "gencap@latest", "--mcp"]
    }
  }
}
```

`--mcp` launches the local MCP stdio server. Any additional arguments after `--mcp` are passed through to the `gencap-mcp` binary.

## Available MCP Tools

The GenCap Control Room MCP server provides tools for managing organisations, projects, issues, workspaces, and task execution.
...
3. **Automatic Issue Generation**: Your MCP client will use the GenCap Control Room MCP server to automatically create structured issues in your project
...
Your MCP client will use the `create_issue` tool to break this down into individual issues and add them to your GenCap Control Room project automatically.
...
### Internal Coding Agents (Within GenCap Control Room)

A powerful workflow involves using coding agents within GenCap Control Room that are also connected to the GenCap Control Room MCP server:
...
4. **Automatic Population**: The agent uses the MCP server to populate individual issues directly in the GenCap Control Room interface
...
Raycast is a popular MCP client that can connect to GenCap Control Room's MCP server. Here's how to configure it:
...
<Frame>
<img src="/images/vk-raycast-mcp-part-1.png" alt="Raycast MCP configuration - adding GenCap Control Room server" />
</Frame>

Configure the GenCap Control Room MCP server in Raycast by adding the server details.
</Tab>
...
Once configured, you'll see the GenCap Control Room MCP server listed and ready to use in Raycast.
</Tab>
</Tabs>
