...
description: "Complete guide to using the GenCap Control Room extension with VSCode, Cursor, and Windsurf"
---

The GenCap Control Room VSCode extension brings task management directly into your IDE, providing seamless integration with logs, diffs, and process monitoring. This extension works with VSCode and popular forks including Cursor and Windsurf.
...
<Card title="VSCode Marketplace" icon="download" href="https://marketplace.visualstudio.com/items?itemName=bloop.gencap">
  Install the official GenCap Control Room extension for VSCode
</Card>

Alternatively, search for the extension ID in VSCode:
1. Open VSCode
2. Press `Ctrl+Shift+X` (Windows/Linux) or `Cmd+Shift+X` (macOS) to open Extensions
3. Search for `@id:bloop.gencap`
4. Click **Install**
</Tab>
...
<Card title="Open VSX Registry" icon="download" href="https://open-vsx.org/extension/bloop/gencap">
...
<Steps>
<Step title="Start a task in GenCap Control Room">
...
<Step title="Launch IDE integration">
  ...
  <Note>
  The button text will reflect your editor choice configured in GenCap Control Room settings.
  </Note>
</Step>
...
**Solution**: Ensure you're working within a worktree created by a GenCap Control Room task.

<Warning>
The extension can only display task information when VSCode is opened in a directory that corresponds to an active GenCap Control Room task worktree.
</Warning>

**Steps to resolve**:
1. Verify you opened VSCode through the "Open in [IDE]" button from a GenCap Control Room task
...
**Problem**: The GenCap Control Room extension doesn't appear in your IDE.

**Solutions**:
- Verify the extension is installed by searching `@id:bloop.gencap` in Extensions
...
<Tip>
**Workflow Optimization**: Always start tasks from the GenCap Control Room web interface before opening in your IDE to ensure proper context and worktree setup.
</Tip>
...
<Info>
The extension is designed to work with any VSCode-compatible editor. If you're using a different VSCode fork, try installing from Open VSX Registry using the extension ID `bloop.gencap`.
</Info>
