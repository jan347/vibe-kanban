...
|----------|-----------------|
| **macOS** | `/var/folders/.../T/gencap/worktrees` (system temp) |
| **Linux** | `/var/tmp/gencap/worktrees` |
| **Windows** | `%TEMP%\gencap\worktrees` |

You can configure a custom location in **Settings → General → Workspace Directory**. When set, worktrees are stored in `{your-path}/.gencap-workspaces`.
...
<Tab title="macOS">
```bash
du -sh $TMPDIR/gencap/worktrees
```
</Tab>
<Tab title="Linux">
```bash
du -sh /var/tmp/gencap/worktrees
```
</Tab>
<Tab title="Windows">
```powershell
Get-ChildItem "$env:TEMP\gencap\worktrees" -Recurse | Measure-Object -Property Length -Sum
```
</Tab>
</Tabs>
...
If you manually delete a worktree directory (e.g., using `rm -rf` in terminal), GenCap Control Room will **automatically recreate it** the next time you open that workspace. The worktree is recreated from the branch's last commit.
...
GenCap Control Room automatically cleans up "orphaned" worktrees on startup - these are worktree directories that no longer have a matching workspace in the database (e.g., if the app crashed during deletion).
