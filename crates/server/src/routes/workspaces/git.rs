...
async fn resolve_gencap_identifier(
...
pub async fn merge_workspace(
    ...
    let vk_id = resolve_gencap_identifier(&deployment, workspace.id).await;
    let commit_message = format!("{} (gencap {})", workspace_label, vk_id);
