...
fn main() -> anyhow::Result<()> {
    ...
    tokio::runtime::Builder::new_multi_thread()
        ...
        .block_on(async move {
            ...
            init_process_logging("gencap-mcp", version);

            let base_url = resolve_base_url("gencap-mcp").await?;
    ...
    I: Iterator<Item = String>,
{
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "-h" | "--help" => {
                println!("Usage: gencap-mcp --mode <global|orchestrator>");
                std::process::exit(0);
            }
            _ => {
                return Err(anyhow::anyhow!(
                    "Unknown argument '{arg}'. Usage: gencap-mcp --mode <global|orchestrator>"
                ));
            }
        }
    }
...
async fn resolve_base_url(log_prefix: &str) -> anyhow::Result<String> {
        ...
        .or_else(|_| std::env::var("PORT"))
    {
        ...
        Err(_) => {
            let port = read_port_file("gencap").await?;
...
fn init_process_logging(log_prefix: &str, version: &str) {
    ...
    tracing::debug!(
        "[{}] Starting GenCap Control Room MCP server version {}...",
        log_prefix,
        version
    );
}
