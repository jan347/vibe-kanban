...
impl ServerHandler for McpServer {
    fn get_info(&self) -> ServerInfo {
        ...
        let preamble = match self.mode() {
            McpMode::Global => {
                "A GenCap Control Room MCP server for task, issue, repository, workspace, and session management."
            }
            McpMode::Orchestrator => {
                "An orchestrator-scoped GenCap Control Room MCP server with tools limited to the configured workspace and orchestrator session context."
            }
        };
        ...
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build())
            .with_server_info(Implementation::new("gencap-mcp", "1.0.0"))
